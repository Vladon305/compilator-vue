...
import { compileMethods } from './compilators/methods/methodsCompilator.js'
import { compileProps } from './compilators/props/propsCompilator.js'
...
function generatePropsCode(node) {
  ...
  switch (node.type) {
    ...
    case 'CallExpression':
      return `${generatePropsCode(node.callee)}(${node.arguments.map(generatePropsCode).join(', ')})`
